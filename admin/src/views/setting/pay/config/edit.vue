    ...
    >
        ...
        >
            <el-form ref="formRef" :model="formData" label-width="9rem" :rules="formRules">
                ...
                <template v-if="formData.pay_way == PayWayEnum.RAINBOW">
                    <el-form-item label="支付网关地址" prop="config.gateway_url">
                        <div>
                            <div style="display: flex; gap: 10px;">
                                <el-input
                                    v-model="formData.config.gateway_url"
                                    placeholder="请输入易支付网关地址（如：http://api.ysf.com）"
                                    style="flex: 1;"
                                />
                                <el-button
                                    type="success"
                            ...
                            <div class="form-tips">易支付平台 API 地址，例如：http://api.ysf.com 或 http://pay.example.com</div>
                        </div>
                    </el-form-item>

                    <el-form-item label="商户 ID" prop="config.app_id">
                        <div>
                            <el-input
                                v-model="formData.config.app_id"
                                ...
                                style="max-width: 250px"
                            />
                            <div class="form-tips">易支付平台分配的商户 ID</div>
                        </div>
                    </el-form-item>

                    <el-form-item label="商户密钥" prop="config.app_secret">
                        <div>
                            <el-input
                                ...
                                style="max-width: 250px"
                            />
                            <div class="form-tips">易支付平台分配的商户密钥</div>
                        </div>
                    </el-form-item>

                    <el-form-item label="支付密钥" prop="config.pay_key">
                        <div>
                            <el-input
                                v-model="formData.config.pay_key"
                                placeholder="请输入易支付支付密钥"
                                style="max-width: 250px"
                            />
                            <div class="form-tips">易支付密钥（用于签名验证）</div>
                        </div>
                    </el-form-item>
                </template>
...
import { getPayConfig, setPayConfig, testPayGateway } from '@/api/setting/pay'
...
const formRules: FormRules = {
    ...
    'config.gateway_url': [
        ...
        {
            pattern: /^https?:\/\/.+/,
            message: '请输入有效的 URL 地址，格式如：http://api.ysf.com',
            trigger: 'blur'
        }
    ]
}
...
const testConnection = async () => {
    if (!formData.value.config.gateway_url) {
        ElMessage.warning('请先填写支付网关地址')
        return
    }
    if (!formData.value.config.app_id) {
        ElMessage.warning('请先填写商户 ID')
        return
    }
    if (!formData.value.config.app_secret) {
        ElMessage.warning('请先填写商户密钥')
        return
    }
    if (!formData.value.config.pay_key) {
        ElMessage.warning('请先填写支付密钥')
        return
    }

    try {
        testing.value = true
        const result = await testPayGateway({
            pay_way: formData.value.pay_way,
            config: formData.value.config
        })
        
        if (result.code === 1) {
            ElMessage.success('连接成功！' + (result.msg || ''))
        } else {
            ElMessage.error('连接失败：' + (result.msg || '未知错误'))
        }
    } catch (error: any) {
