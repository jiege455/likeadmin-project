...
const axiosHooks: AxiosHooks = {
    ...
    async responseInterceptorsHook(response) {
        ...
        // 前端配置的 show 可以覆盖后端返回的 show
        const frontendShow = response.config.requestOptions?.show
        const shouldShow = frontendShow !== undefined ? frontendShow : show
        switch (code) {
