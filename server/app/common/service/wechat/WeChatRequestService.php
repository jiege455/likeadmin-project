...
class WeChatRequestService extends BaseLogic
{
    ...
    public static function getUserAuthByCode($code)
    {
        ...
        try {
            $requests = Requests::get($url, [], ['timeout' => 10]);
            $result = json_decode($requests->body, true);
            if (isset($result['errcode']) && $result['errcode'] != 0) {
                Log::error('WeChat Auth Error: ' . ($result['errmsg'] ?? 'Unknown error'));
                return null;
            }
            return $result;
        } catch (\Exception $e) {
            Log::error('WeChat Request Error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public static function getUserInfoByAuth($accessToken, $openId)
    {
        ...
        try {
            ...
            if (isset($result['errcode']) && $result['errcode'] != 0) {
                Log::error('WeChat UserInfo Error: ' . ($result['errmsg'] ?? 'Unknown error'));
                return null;
            }
            return $result;
        } catch (\Exception $e) {
