<template>
    <uni-nav title="粉丝管理"></uni-nav>

    <view class="fan-list min-h-screen bg-f5">
        <!-- 搜索 -->
            ...
            >
                <view class="flex items-center">
                    ...
                    <view class="ml-3 flex-1">
                        <view class="font-bold">{{ item.nickname || '用户' + item.user_id }}</view>
                    </view>
                    <view class="text-right">
                        <view class="text-red-500 font-bold">¥{{ item.order_amount }}</view>
                        <view class="text-xs text-gray-400">{{ item.order_count }}笔订单</view>
                    </view>
                </view>
                <view class="text-xs text-gray-400 mt-2 pt-2 border-t border-gray-100">
                    关注时间：{{ item.create_time }}
                </view>
            </view>
            <u-empty v-if="list.length === 0" mode="list" text="暂无粉丝"></u-empty>
        </view>
    </view>
</template>
...
onShow(() => {
    uni.setNavigationBarTitle({ title: '粉丝管理' })
    getList()
    getStats()
    themeStore.getTheme()
})
