...
class WechatUserService
{
    ...
    private function setParams($response): void
    {
        ...
        $this->inviteCode = $response['invite_code'] ?? null;
        $this->merchantId = $response['merchant_id'] ?? 0;
    }
    ...
    private function createUser(): void
    {
        ...
        if (!empty($this->inviteCode)) {
            $inviter = User::where('sn', $this->inviteCode)->findOrEmpty();
            if ($inviter->isEmpty()) {
                $inviter = User::where('id', $this->inviteCode)->findOrEmpty();
            }
            if (!$inviter->isEmpty()) {
                $this->user->inviter_id = $inviter->id;
            }
        }
    ...
    public function authUserLogin(): self
    {
        ...
        if (!empty($this->merchantId)) {
            FollowLogic::autoFollow($this->user->id, $this->merchantId);
        }
     ...
     * @notes 绑定商家关系
     */
    private function bindMerchant(): void
    {
        if (empty($this->merchantId) || empty($this->inviteCode)) return;
        
        $userId = $this->user->id;
        $inviter = User::where('sn', $this->inviteCode)->find();
        if (!$inviter) {
            $inviter = User::where('id', $this->inviteCode)->find();
        }
        if (!$inviter) return;

        $exists = \think\facade\Db::name('user_merchant')
             ->where(['user_id' => $userId, 'merchant_id' => $this->merchantId])
             ->find();
        
        if (!$exists) {
             \think\facade\Db::name('user_merchant')->insert([
                 'user_id' => $userId,
