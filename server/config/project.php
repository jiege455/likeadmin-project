...
return [
    ...
    // 后台管理员token（登录令牌）配置
    'admin_token' => [
        'expire_duration' => 3600 * 24 * 7,//管理后台token过期时长(单位秒）调整为7天
        'be_expire_duration' => 3600 * 24,//管理后台token临时过期前时长，自动续期
    ],

    // 商城用户token（登录令牌）配置
    'user_token' => [
        'expire_duration' => 3600 * 24 * 7,//用户token过期时长(单位秒）调整为7天
        'be_expire_duration' => 3600 * 24,//用户token临时过期前时长，自动续期
    ],
    ...
    // WebSocket配置
    'websocket_url' => env('project.websocket_url', 'ws://127.0.0.1:8282'),
