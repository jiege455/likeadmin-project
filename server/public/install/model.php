...
//        }
    ...
    {
        $this->host = $post['host'];
        $this->name = $this->sanitizeDbName($post['name']);
        $this->user = $post['user'];
        $this->encoding = 'utf8mb4';
        $this->password = $post['password'];
        $this->port = $post['port'];
        $this->prefix = $this->sanitizeDbName($post['prefix']);
        $this->clearDB = $post['clear_db'] == 'on';
    }

    /**
     * Notes: 过滤数据库名和表前缀，防止SQL注入
     * @param $name
     * @return string
     */
    private function sanitizeDbName($name)
    {
...
//            }
    ...
    public function initAccount($post)
    {
        ...
        // 更新首页装修配置 - 只显示"当前商家信息"组件
        // 开发者：杰哥网络科技 QQ: 2711793818 杰哥
        $homepageData = '[{"title":"当前商家信息","name":"current-merchant","content":{"show_switch":true,"show_actions":true,"show_tabs":true,"show_search":true,"show_content_list":true,"show_coupon":true,"show_apply_btn":true,"show_has_merchant":true},"styles":{"primary_color":"#FF2D3A","title_color":"#333","desc_color":"#666","margin_top":0,"margin_bottom":10,"padding_horizontal":12,"border_radius":12}}]';
        $homepageMeta = '[{"title":"页面设置","name":"page-meta","content":{"title":"首页","bg_type":"1","bg_color":"#F5F5F5","bg_image":"","text_color":"1","title_type":"1","title_img":""},"styles":{}}]';
        $sql .= "UPDATE `la_decorate_page` SET `data`='{$homepageData}', `meta`='{$homepageMeta}', `update_time`='{$time}' WHERE `type`=1;";
