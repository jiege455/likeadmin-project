...
// 监听 themeColorId 变化，强制同步预设颜色
watch(
    () => formData.value.themeColorId,
    (val) => {
        const item = themeColors.value.find((item) => item.id == val)
        // 如果是预设主题（ID不为7），且找到了对应配置，强制更新核心主题色
        if (item && item.id != 7) {
             formData.value.themeColor1 = item.color1
             formData.value.themeColor2 = item.color2
        }
    },
    { immediate: true } // 立即执行，确保组件挂载时也就位
)
</script>
