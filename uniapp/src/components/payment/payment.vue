                    ...
                    <view class="payment h-full w-full flex flex-col">
                        ...
                        <view class="main flex-1 mx-[20rpx]">
                            <view>
                                <view class="payway-lists">
                                    <!-- 空状态提示 -->
                                    <view v-if="payData.lists.length === 0" class="flex flex-col items-center justify-center py-20">
                                        <u-empty mode="list" text="暂无可用支付方式"></u-empty>
                                        <view class="text-xs text-gray-400 mt-4 text-center px-4">
                                            请前往后台管理 → 设置 → 支付设置 → 支付方式中开启支付方式
                                        </view>
                                    </view>
                                    <u-radio-group v-else v-model="payWay" class="w-full">
                                        <view
                                            class="p-[20rpx] flex items-center w-full payway-item"
                                            :class="{
                                                'payway-item-disabled':
                                                    item.pay_way == 1 &&
                                                    userBalance < payData.order_amount
                                            }"
                        ...
                        <view class="submit-btn p-[20rpx]">
                            <u-button
                                ...
                                type="primary"
                                :loading="isLock"
                                :disabled="payData.lists.length === 0"
                            >
                                {{ payData.lists.length === 0 ? '暂无可用支付方式' : '立即支付' }}
                            </u-button>
                        </view>
                    </view>
                </template>
...
const getPayData = async () => {
    ...
    try {
        ...
        const balanceItem = payData.value.lists.find((item: any) => item.pay_way === 1)
        if (balanceItem) {
        ...
        if (checkPay && checkPay.pay_way === 1 && userBalance.value < payData.value.order_amount) {
            const otherPayWay = payData.value.lists.find((item: any) => item.pay_way !== 1)
            payWay.value = otherPayWay ? otherPayWay.pay_way : checkPay.pay_way
        } else {
...
const selectPayWay = (item: any) => {
    if (item.pay_way === 1 && userBalance.value < payData.value.order_amount) {
        uni.showToast({ title: '余额不足，请选择其他支付方式', icon: 'none' })
        return
    }
    payWay.value = item.pay_way
}
const payment = (() => {
    // 查询是否绑定微信
...
const { isLock, lockFn: handlePay } = useLockFn(async () => {
    if (payWay.value === 1 && userBalance.value < payData.value.order_amount) {
        uni.showToast({ title: '余额不足，请选择其他支付方式', icon: 'none' })
        return
    }
    try {
