<template>
    <el-form label-width="70px">
        ...
        <el-card shadow="never" class="!border-none flex mt-2" v-if="formData.style == 1">
            <div class="flex items-end mb-4">
                <div class="text-base text-[#101010] dark:text-[#ffffff] font-medium">背景联动</div>
            </div>
            <el-radio-group v-model="formData.bg_style">
        ...
        <el-card shadow="never" class="!border-none flex-1 mt-2">
            <div class="flex items-end">
                ...
                <div v-if="formData.style == 1" class="text-xs text-tx-secondary ml-2">
                        ...
                        <del-wrap :key="index" @close="handleDelete(index)" class="w-full">
                            <div class="bg-fill-light w-full p-4 mt-4">
                                <div class="flex justify-center w-full">
                                    <template v-if="formData.style == 1">
                                        <material-picker
                                            size="122px"
                                            v-model="item.image"
            ...
            <div class="mt-4" v-if="formData.data?.length < limit">
...
const formData = ref(cloneDeep(props.content))
...
const handleAdd = () => {
    if (formData.value.data?.length < limit) {
...
const handleDelete = (index: number) => {
    if (formData.value.data?.length <= 1) {
