...
class ArticleController extends BaseApiController
{
    ...
    public function detail()
    {
        ...
        $merchantId = $this->request->get('merchant_id/d', 0);

        if ($this->userId && !empty($inviteCode)) {
            \app\common\logic\DistributionLogic::bindInviter($this->userId, $inviteCode, $merchantId);
        }
    ...
    {
        $articleId = $this->request->post('id/d');
        $result = ArticleLogic::buy($articleId, $this->userId);
        return $this->data($result);
    }
}