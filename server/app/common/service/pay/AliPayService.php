...
//        $options->gatewayHost = 'openapi.alipaydev.com'; //测试沙箱地址
        ...
        try {
            ...
            switch ($data['passback_params']) {
                case 'recharge':
                    ...
                    break;
                case 'article':
                    $order = ArticleOrder::where(['order_sn' => $data['out_trade_no']])->findOrEmpty();
                    if ($order->isEmpty() || $order->pay_status == PayEnum::ISPAID) {
                        return true;
                    }
                    PayNotifyLogic::handle('article', $data['out_trade_no'], $extra);
                    break;
            }
    ...
    public function getPayDesc($from)
    {
        ...
        $configKey = [
            'order' => 'order_desc',
            'recharge' => 'recharge_desc',
            'article' => 'article_desc',
        ];
        
        $configName = $configKey[$from] ?? '';
        if ($configName) {
            $configValue = \think\facade\Db::name('config')
                ->where('type', 'pay_desc')
