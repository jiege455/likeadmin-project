...
class LoginMiddleware
{
    ...
    public function handle($request, \Closure $next)
    {
        ...
        // 开发者公众号：杰哥网络科技 qq2711793818 杰哥
        // 检查用户是否被禁用或删除
        if (!empty($userInfo) && isset($userInfo['is_disable']) && $userInfo['is_disable'] == 1) {
            return JsonService::fail('账号已被禁用，请联系客服', [], -1, 0);
        }
        ...
        if ($userInfo) {
            ...
            if (time() > ($userInfo['expire_time'] - $beExpireDuration)) {
                ...
                if (!empty($result)) {
                    ...
                    Log::error('Token 续期失败：user_id=' . ($userInfo['user_id'] ?? 'unknown'));
                }
            }
        }
        ...
        // 如果需要刷新token，在响应头中添加新token
        if ($refreshToken && !empty($newToken)) {
            $response->header('X-Refresh-Token', $newToken);
        }
